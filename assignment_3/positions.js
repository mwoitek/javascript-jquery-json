...
function cant_add_position() {
    if (unused_positions.length == 0) {
        alert(
            "Maximum of " + maximum_number_entries + " position entries exceeded"
        );
        return true;
    }
    return false;
...
function print_position_div(pos) {
    ...
    div_string +=
        '<input type="button" value="-" onclick="remove_position(' +
        pos +
        '); return false;"></p>\n';
    div_string +=
        '<textarea name="desc' + pos + '" rows="8" cols="80"></textarea>\n</div>\n';
    return div_string;
}
